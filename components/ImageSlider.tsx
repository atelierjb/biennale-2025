...
import type { SlideImage } from '@/lib/exhibitors/types'
...
export default function ImageSlider({ slides, labels, isPortrait = false, className = '' }: Props) {
  ...
  // Horizontal swipe. Without it a sideways drag here does nothing locally and
  // reaches the document, where the browser reads it as back/forward navigation.
  ...
  const onTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      // Only act on a clearly horizontal gesture, so vertical scrolling is unaffected.
  ...
  return (
    <div
      className={`relative w-full overflow-hidden${isPortrait ? ' portrait' : ''} ${className}`}
      style={{ touchAction: 'pan-y' }}
      onTouchStart={onTouchStart}
      onTouchEnd={onTouchEnd}
    >
      {/* Dots */}
      {/* Plain buttons: role="tab" without matching tabpanels is invalid ARIA. */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex z-100 justify-center gap-2 mt-3">
        {slides.map((_, i) => (
          <button
            key={i}
            type="button"
