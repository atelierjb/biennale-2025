...
type Props = {
  /** Required: an optional prop here silently fell back to Danish. */
...
export default function StackedNav({ labels }: Props) {
  ...
  useEffect(() => {
    ...
    const floater = nav.querySelector<HTMLDivElement>('.stacked-nav__float')!
    if (!topList || !bottomList || !floater) return

    // Widened to string: the ids are compared against raw values read back out
    // of the DOM (dataset.target, href hashes), which are plain strings.
    const order: string[] = NAV_ITEMS.map(n => n.id)
    ...
    return () => {
      ...
      // This effect physically relocates <li>s between the two lists. Put them
      // back so a re-run (React's dev double-invoke, or a remount reusing the
      // same nodes) starts from the markup it expects instead of a half-sorted
      // list.
      hideFloater()
      order.forEach(moveToBottom)
      Object.values(items).forEach(li => li.classList.remove('is-hidden'))
    }
  }, [])
